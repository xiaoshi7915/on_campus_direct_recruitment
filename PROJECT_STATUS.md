...
- [x] 查询报名学生 ✅
- [x] 邀请企业 ✅
- [x] 双选会宣讲会流程审批 ✅（后端API + 前端集成）（后端API + 前端集成）
...
#### 3. 企业信息管理
- [x] 企业信息编辑 ✅
- [x] 宣传页预览 ✅（前端页面已创建，导出PDF功能待集成PDF生成库）

#### 4. 账号管理/子账号管理
- [x] 由主账号对子账号进行管理 ✅（后端API + 前端页面）

#### 5. 消息中心
- [x] 聊天消息 ✅
- [x] 系统消息 ✅（后端API + 前端页面）
- [x] 日程表提醒消息 ✅（Schedule模型已有reminder_time字段，前端页面已实现）
...
- [x] 收藏简历 ✅
- [x] 标记简历 ✅（后端API + 前端集成）
- [x] 分享简历 ✅（前端实现，支持Web Share API和复制链接）
- [x] 发送面试邀请 ✅
- [ ] 发送宣讲会邀请（需要新增邀请功能）
- [x] 发起聊天 ✅
...
- [x] 展示职位数据 ✅
- [x] 关键词搜索 ✅
- [x] 设置关键标签 ✅（Job模型已有tags字段，前端JobEdit.vue已支持）
...
- [x] 系统日程数据展示 ✅
- [x] 手动输入日程展示 ✅
- [x] 日程标记 ✅（已有API支持：/schedules/{id}/complete）
- [x] 日程提醒：面试安排、双选会、宣讲会等日程 ✅（Schedule模型已有reminder_time字段，前端页面已实现）

#### 11. 数据报表
- [x] 企业数据报表 ✅（后端API + 前端页面）

#### 12. 人才管理
- [x] 企业触达过的人才库：全部、收藏、沟通中、已约面、已录用 ✅（后端API + 前端页面）
...
- [x] 日程展示和管理 ✅
- [x] 日程标记 ✅（已有API支持：/schedules/{id}/complete）
- [x] 日程提醒 ✅（Schedule模型已有reminder_time字段）
...
#### 15. 数据报表
- [x] 个人数据统计 ✅（已有API：/statistics/student/personal）
