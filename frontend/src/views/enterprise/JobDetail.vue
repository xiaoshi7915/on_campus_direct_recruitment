<template>
  <div class="enterprise-job-detail">
    ...
    <div v-else>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        <h2 class="text-xl font-semibold mb-4">职位申请列表</h2>
        ...
        <div v-else>
          ...
          >
            <div class="flex justify-between items-start">
              <div>
                <h3 class="font-semibold">{{ application.student_name || '学生' }}</h3>
                <p class="text-sm text-gray-600">申请时间：{{ formatDate(application.created_at) }}</p>
                <p class="text-sm">状态：{{ application.status }}</p>
              </div>
              <button
