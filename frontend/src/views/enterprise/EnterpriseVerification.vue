<template>
  <div class="enterprise-verification">
    <div class="mb-6">
      <h1 class="text-3xl font-bold">企业认证</h1>
      <p class="text-gray-600 mt-2">提交企业认证材料，通过审核后可获得认证标识</p>
    </div>
      ...
      >
        <div class="flex items-center">
          ...
          <div>
            ...
            <p class="text-green-600 text-sm mt-1">您的企业认证已通过审核</p>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
          <label class="md:col-span-1 text-sm font-medium text-gray-700 pt-2">
            营业执照 <span class="text-red-500">*</span>
          </label>
          <div class="md:col-span-2">
            <input
              ...
              type="file"
              accept="image/*,.pdf"
              @change="handleFileUpload('business_license', $event)"
              class="hidden"
            />
            <div class="flex flex-col space-y-2">
              <button
                type="button"
                @click="$refs.businessLicenseInput.click()"
                class="w-full md:w-auto px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-left"
              >
                选择文件
              </button>
              <div v-if="form.business_license_url" class="flex items-center space-x-3">
                ...
                <a
                  :href="form.business_license_url"
                  target="_blank"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
          <label class="md:col-span-1 text-sm font-medium text-gray-700 pt-2">
            法人身份证正面 <span class="text-red-500">*</span>
          </label>
          <div class="md:col-span-2">
            <input
              ref="legalPersonIdFrontInput"
              type="file"
              accept="image/*"
              @change="handleFileUpload('legal_person_id_front', $event)"
              class="hidden"
            />
            <div class="flex flex-col space-y-2">
              <button
                type="button"
                @click="$refs.legalPersonIdFrontInput.click()"
                class="w-full md:w-auto px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-left"
              >
                选择文件
              </button>
              <div v-if="form.legal_person_id_front_url" class="flex items-center space-x-3">
                <span class="text-sm text-green-600 flex items-center">
                  <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                  </svg>
                ...
                <a
                  :href="form.legal_person_id_front_url"
                  target="_blank"
                  class="text-sm text-blue-600 hover:underline"
                >
                  查看文件
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
          <label class="md:col-span-1 text-sm font-medium text-gray-700 pt-2">
            法人身份证反面 <span class="text-red-500">*</span>
          </label>
          <div class="md:col-span-2">
            <input
              ref="legalPersonIdBackInput"
              type="file"
              accept="image/*"
              @change="handleFileUpload('legal_person_id_back', $event)"
              class="hidden"
            />
            <div class="flex flex-col space-y-2">
              <button
                type="button"
                @click="$refs.legalPersonIdBackInput.click()"
                class="w-full md:w-auto px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-left"
              >
                选择文件
              </button>
              <div v-if="form.legal_person_id_back_url" class="flex items-center space-x-3">
                <span class="text-sm text-green-600 flex items-center">
                  <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                  </svg>
                ...
                <a
                  :href="form.legal_person_id_back_url"
                  target="_blank"
                  class="text-sm text-blue-600 hover:underline"
                >
                  查看文件
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
          ...
          <div class="md:col-span-2">
            ...
            <div class="flex flex-col space-y-2">
              <button
                type="button"
                @click="$refs.authorizationLetterInput.click()"
                class="w-full md:w-auto px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-left"
              >
                选择文件
              </button>
              <div v-if="form.authorization_letter_url" class="flex items-center space-x-3">
                ...
                <a
                  :href="form.authorization_letter_url"
                  target="_blank"
                  class="text-sm text-blue-600 hover:underline"
                >
                  查看文件
        ...
        <div class="flex justify-end space-x-4 pt-4 border-t">
          ...
          <button
            type="submit"
            :disabled="submitting || !form.business_license_url || !form.legal_person_id_front_url || !form.legal_person_id_back_url"
...
import {
  ...
  type EnterpriseVerificationCreateRequest
} from '@/api/verifications'
import { uploadDocument } from '@/api/upload'

const currentVerification = ref<EnterpriseVerification | null>(null)
const showForm = ref(false)
const submitting = ref(false)
const otherDocuments = ref<string[]>([])

const form = ref<EnterpriseVerificationCreateRequest>({
  business_license_url: undefined,
  legal_person_id_front_url: undefined,
  legal_person_id_back_url: undefined,
...
const loadVerification = async () => {
  try {
    const response = await getEnterpriseVerifications({ page_size: 1 })
  ...
  try {
    ...
    if (response.url) {
      if (field === 'business_license') {
        form.value.business_license_url = response.url
      } else if (field === 'legal_person_id_front') {
        form.value.legal_person_id_front_url = response.url
      } else if (field === 'legal_person_id_back') {
        form.value.legal_person_id_back_url = response.url
...
const handleSubmit = async () => {
  if (!form.value.business_license_url || !form.value.legal_person_id_front_url || !form.value.legal_person_id_back_url) {
    alert('请上传必填材料')
    return
  }
  ...
  submitting.value = true
  try {
    await createEnterpriseVerification(form.value)
    alert('认证申请已提交，请等待审核')
    showForm.value = false
